// src/modules/ordenes/Ordenes.tsx
...
export function Ordenes({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const [estado, setEstado] = useState<"todos" | "recepcion" | "diagnostico" | "reparacion" | "listo" | "entregado">(
    "todos",
  )
  ...
  const filtered = useMemo(() => {
    ...
    const sorter = {
      creada_desc: (a: OrdenRow, b: OrdenRow) => +new Date(b.creada) - +new Date(a.creada),
      creada_asc: (a: OrdenRow, b: OrdenRow) => +new Date(a.creada) - +new Date(b.creada),
      act_desc: (a: OrdenRow, b: OrdenRow) => +new Date(b.actualizada) - +new Date(a.actualizada),
      act_asc: (a: OrdenRow, b: OrdenRow) => +new Date(a.actualizada) - +new Date(b.actualizada),
    }[ordenar]
    return [...list].sort(sorter)
  }, [rows, q, estado, desde, hasta, ordenar])

  // --- Agrupación por cliente (cliente+teléfono) con colapsado ---
  const [expanded, setExpanded] = useState<Record<string, boolean>>({})
  const groups = useMemo(() => {
    const m = new Map<string, { firstId: string; count: number }>()
    for (const r of filtered) {
      const key = `${r.cliente}#${r.telefono}`
      const g = m.get(key)
      if (!g) m.set(key, { firstId: r.id, count: 1 })
      else g.count++
    }
    return m
  }, [filtered])
                  ...
                  return (
                    <tr key={r.id} className="border-t border-neutral-200/70 dark:border-neutral-800">
                      ...
                      <td className="py-2 pr-3">
                        {isFirst && more > 0 && (
                          <button
                            className="mr-2 px-2 py-0.5 text-xs rounded-lg border border-neutral-300/60 dark:border-neutral-700"
                            title={isExpanded ? "Ocultar resto" : "Mostrar resto"}
