# apply_data_tools.ps1 — Exportar CSV + Importar JSON + Imprimir (estilos print)
$ErrorActionPreference='Stop'
Set-Location 'C:\gestor-reparaciones\apps\web'

# ------- Actualiza Ordenes.tsx (export CSV + import JSON) -------
@'
...
# ------- Actualiza DetalleOrden.tsx (botón Imprimir) -------
(Get-Content src\modules\reparacion\DetalleOrden.tsx -Raw) `
  -replace '(<button className="btn" onClick={exportarJSON}>Exportar JSON</button>)',
           '$1 <button className="btn" onClick={()=>window.print()}>Imprimir</button>' |
  Set-Content src\modules\reparacion\DetalleOrden.tsx -Encoding utf8

# ------- Añade estilos de impresión a src/styles.css -------
$printCss = @'
/* === PRINT ONLY === */
@media print {
  html, body { background: white !important; color: black !important; }
  .dark * { color: black !important; background: white !important; }
  .btn, .tab, aside, header { display: none !important; }
  .card, .card-body { border: 1px solid #000 !important; box-shadow: none !important; }
  .max-w-6xl { max-width: 100% !important; }
  a[href]:after { content: " (" attr(href) ")"; font-size: 0.8em; }
}
'@
Add-Content src\styles.css $printCss -Encoding utf8
